...
*   **Простая настройка**: Python, `requests` и Reticulum (`rns`). Poetry для зависимостей и тестов.
...
*   **Повторное использование соединений**: HTTP/1.1 keep-alive и пул из одного TCP-соединения по умолчанию.
...
*   `pool_connections` / `pool_maxsize` (по умолчанию `1` для одного долгоживущего TCP-сеанса)
*   `keepalive_timeout` на сервере (по умолчанию `60`)
